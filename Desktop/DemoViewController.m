...
@interface DemoViewController() <FastTextViewDelegate>
{
...
//    self.automaticallyAdjustsScrollViewInsets = NO;
    ...
    if (_fastTextView==nil) {
        ...
        FastTextView *view = [[FastTextView alloc] initWithFrame:CGRectMake(-7, origin_y , SCREEN_WIDTH, SCREEN_HEIGHT - origin_y)];
...
- (void) postMessage {
    ...
    //    _modelMessage = [[RRMessageModel alloc] init];
    //    _modelMessage.text = _textView.text;
    //    _modelMessage.photos = _selectedPhotos;
...
//            [_contentArray addObject:answerContent];
    ...
    NSLog(@"%@,******************%ld",mutableAttributedString,mutableAttributedString.length);
...
//    NSString *answerContent = [NSString stringWithFormat:@"<p>%@</p>",_textView.text];
//    NSMutableString *answerString = [NSMutableString stringWithString:answerContent];
...
- (void)_addAttachmentFromAsset:(ALAsset *)asset;
{
    ...
    if ([rep getBytes:[data mutableBytes] fromOffset:0 length:[rep size] error:&error] == 0) {
        ...
        if(isAddSlide){
            ...
            float scale = thumbimg.size.width*1.0/(SCREEN_WIDTH - 22);
            cell.thumbImageWidth=SCREEN_WIDTH - 22;
